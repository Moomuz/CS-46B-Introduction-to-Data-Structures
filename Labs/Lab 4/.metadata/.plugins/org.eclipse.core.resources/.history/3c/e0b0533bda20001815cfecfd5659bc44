...
public class DotDisplay extends JFrame implements ActionListener
{
	...
	private void read(File in) throws IOException, DotException
	{
		FileReader fr = new FileReader(in);
		BufferedReader br = new BufferedReader(fr);
		DotReader dr = new DotReader(br);				//converts file into something DotReader can read
		
		boolean isNotNull = false;
		while(isNotNull == false) //can also use (!isNotNull) in place of "isNotNull is not false"
		{
			Dot temp = dr.readDot();				
			if(temp == null)					
			{
