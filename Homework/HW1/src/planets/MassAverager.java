...
public class MassAverager 
{
	...
	{
		float sum = 0;
		for(Planet pl: Planet.getAll()) //each planet object, for planet.getAll() times
		{									//no need to create new planet object
			sum += pl.getMass();
		}
		return sum/Planet.getAll().length; //sum/number of planets
	}
